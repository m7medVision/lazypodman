...
func chineseSet() TranslationSet {
	return TranslationSet{
		...
		ErrorOccurred:                     "发生错误！请在 https://github.com/m7medVision/lazypodman/issues 上创建一个问题",
		ConnectionFailed:                  "无法连接到 Podman 客户端。您可能需要重新启动 Podman 客户端",
		UnattachableContainerError:        "容器不支持 attaching。您必须使用“-it”标志运行服务，或者在compose.yml文件中使用`stdin_open: true，tty: true`",
		WaitingForContainerInfo:           "在 Podman 给我们更多关于容器的信息之前，无法继续。请几分钟后重试。",
		CannotAttachStoppedContainerError: "您不能 attach 到已停止的容器，您需要先启动它（您可以用 'r' 键来执行此操作）（是的，我懒得为您自动执行此操作）（很酷的是，我可以通过错误消息与您进行一对一的通讯）",
		CannotAccessDockerSocketError:     "无法访问 Podman 套接字：unix:///var/run/podman.sock\n请以 root 用户身份运行 lazypodman 或阅读https://docs.podman.com/install/linux/linux-postinstall/",
		...
		ConfigTitle:               "配置",
		EnvTitle:                  "环境变量",
		DockerComposeConfigTitle:  "Compose配置",
		TopTitle:                  "系统资源管理",
		StatsTitle:                "统计信息",
		CreditsTitle:              "关于我们",
